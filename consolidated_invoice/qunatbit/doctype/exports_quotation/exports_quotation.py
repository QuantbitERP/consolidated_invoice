...
class ExportsQuotation(SellingController):
	...
	def on_submit(self):
		...
		# self.update_opportunity("ExportsQuotation")
		# self.update_lead()

	def on_cancel(self):
		if self.lost_reasons:
			self.lost_reasons = []
		super(ExportsQuotation, self).on_cancel()
...
def make_sales_order(source_name, target_doc=None):
	quotation = frappe.db.get_value(
		"ExportsQuotation", source_name, ["transaction_date", "valid_till"], as_dict=1
	)
...
def _make_sales_order(source_name, target_doc=None, ignore_permissions=False):
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			"ExportsQuotation": {"doctype": "Sales Order", "validation": {"docstatus": ["=", 1]}},
			"Quotation Item": {
...
def set_expired_status():
	...
	cond = "`tabExportsQuotation`.docstatus = 1 and `tabExportsQuotation`.status NOT IN ('Expired', 'Lost') and `tabExportsQuotation`.valid_till < %s"
	...
	frappe.db.multisql(
		{
			"mariadb": """UPDATE `tabExportsQuotation`  SET `tabExportsQuotation`.status = 'Expired' WHERE {cond} and not exists({so_against_quo})""".format(
				cond=cond, so_against_quo=so_against_quo
			),
			"postgres": """UPDATE `tabExportsQuotation` SET status = 'Expired' FROM `tabSales Order`, `tabSales Order Item` WHERE {cond} and not exists({so_against_quo})""".format(
				cond=cond, so_against_quo=so_against_quo
			),
		},
		(nowdate()),
...
def _make_sales_invoice(source_name, target_doc=None, ignore_permissions=False):
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			"ExportsQuotation": {"doctype": "Sales Invoice", "validation": {"docstatus": ["=", 1]}},
...
def _make_customer(source_name, ignore_permissions=False):
	quotation = frappe.db.get_value(
		"ExportsQuotation", source_name, ["order_type", "party_name", "customer_name"], as_dict=1
	)
